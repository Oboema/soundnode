    ...
    function listen(){
        _this.io.sockets.on('connection', function(socket){
            ...
            socket.on( 'disconnect',   function(){
                ...
                // if player, pop player from list, inform connected clients
            });
            
            // add the player to the list of players
            socket.on('player logon', function(data){
                var player = data.player;
                socket.users = [];  // keep a list of connected users in the player socket object
            ...
            socket.on( 'user to player signup', function(data){
                ...
                console.log('user ['+socket.config.user+'] want to sign up to player ['+data.player+']');

                _this.players.forEach(function(player){
                    if (player.name === data.player){
                        socket.player = player.socket;

                        socket.emit('user to player signup', { 'player': player.name });
                        player.socket.users.push(
                            {   'name'      : data.user,
                                'socket'    : socket
                            });

                        player.socket.emit('load sounds', { 'config' : socket.config });
                    }
                });
            });
            ...
            socket.on('sound stopped', function(data){
                console.log('sound ['+data.sound+'] from user ['+data.user+'] has ended');

                //console.log('number of connected users: ['+ socket.users.length +']');
                socket.users.forEach(function(user){
                    if(user.name == data.user){
                        user.socket.emit('sound stopped', {sound    : data.sound});
                    }
                });
            });
