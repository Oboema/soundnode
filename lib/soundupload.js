...
module.exports  = function(){
    ...
    function initUser(user){
        var user_dir    = path.join(user_sound_dir, user),
            user_config = path.join(user_config_dir, user+'.json');


        // Create user dir if it's not there
        try{
            var stats = fs.statSync( user_dir); 
        ...
        try{
            ...
            fs.writeFileSync(user_config, JSON.stringify(bare_user_config));
        }
    }
    // TODO :Move all the config file code to a different library file.
    // Pass a user config object to this file, so we have a cleaner
    // separation between sound file uploading and config processing.
    // Also, we don't have to open the damn config file for every transaction
    function addSoundToConfig(user, title, file_hash){
    ...
    function writeSound(file, user, title){
        ...
        addSoundToConfig(user, title, hash);
    ...
    app.post('/soundupload',    function(req, res){
        ...
        busboy.on('file', function(fieldname, file, filename, encoding, mimetype){
            ...
            if (user) {     // Only write the sound file if we have a user
                sound_dest  = writeSound(file, user, title);
            }
